...
`@septic` use function calling to perform different actions. The different commands acts as agents with a given set of functions that it can call. A function can be used to gather more context (e.g. get documentation of all functions), perform non-modifying actions (e.g. validate a given calculation) or perform modifying actions (e.g. add/delete a row to an scg source). The participant will always ask for permission before performing a modifying action that the user can decline, while all functions related to gathering context and non-modifying actions will be called without asking for permission.
