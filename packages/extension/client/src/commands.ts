...
export function registerCommandDetectCycles(
...
export function registerCommandCompareCnfg(
    context: vscode.ExtensionContext,
    client: LanguageClient,
) {
    vscode.commands.registerCommand("septic.compareCnfg", async () => {
        ...
        const wsedit = new vscode.WorkspaceEdit();
        const wsPath = vscode.workspace.workspaceFolders![0].uri.fsPath;
        const filePath = vscode.Uri.file(wsPath + `/comparison.md`);
        wsedit.createFile(filePath, {
...
export function registerCommandOpcTagList(
    context: vscode.ExtensionContext,
    client: LanguageClient,
) {
...
export function registerCommandAddTemplate(
    scgTreeProvider: ApplicationTreeProvider,
) {
    vscode.commands.registerCommand(
        ...
        async (node: ApplicationTreeItem) => {
            ...
            const sources = (await node.config.getSources()).map((s) => ({
                label: s.id,
                value: s.id,
            }));
            const source = await vscode.window.showQuickPick(
                [{ label: "No source", value: undefined }, ...sources],
                { title: "Select source for template" },
            );
            if (!template || !source) {
                return;
            }
            const elementToInsertAfter =
                node.type === ApplicationTreeItemType.Template
                    ? node.label
                    : undefined;
            node.config.addTemplate(
                template,
                source.value,
            ...
            const filePath = vscode.Uri.file(
                node.config.templatepath + "/" + template,
            );
            wsedit.createFile(filePath);
            await vscode.workspace.applyEdit(wsedit);
            await vscode.window.showTextDocument(filePath, {
...
export function registerCommandRemoveTemplate(
    applicationTreeProvider: ApplicationTreeProvider,
) {
    vscode.commands.registerCommand(
...
export function registerCommandRenameTemplate(
    applicationTreeProvider: ApplicationTreeProvider,
) {
    vscode.commands.registerCommand(
        ...
        async (node: ApplicationTreeItem) => {
            ...
            await vscode.workspace.fs.rename(
                vscode.Uri.file(node.config.templatepath + "/" + node.label),
                vscode.Uri.file(node.config.templatepath + "/" + newName),
                { overwrite: false },
            );
...
export function registerCommandAddSource(
    applicationTreeProvider: ApplicationTreeProvider,
) {
    vscode.commands.registerCommand(
        ...
        async (node: ApplicationTreeItem) => {
            ...
            const fileName = await vscode.window.showInputBox({
                prompt: "Enter source file name (with path relative to scg folder)",
            });
            if (!fileName) {
                return;
            }
            ...
            const fileUri = vscode.Uri.file(
                path.join(path.dirname(node.config.path), fileName),
            );
...
export function registerCommandRefreshApplications(
    applicationManager: SepticApplicationManager,
) {
    vscode.commands.registerCommand(
        "septic.applicationTree.refresh",
        async () => {
...
export function registerCommandRefreshFunctions(
    functionTree: SepticFunctionTreeProvider,
) {
...
export function registerCommandStartApplication(
    applicationManager: SepticApplicationManager,
) {
    vscode.commands.registerCommand(
        "septic.applicationTree.start",
        async (e: ApplicationTreeItem) => {
            const application = await applicationManager.getApplicationByName(
                e.label,
            );
            if (!application) {
                return;
            }
            const runFolderPath = vscode.Uri.joinPath(
                vscode.Uri.file(application.path),
                "run",
            );
            const makeAndStartPath = vscode.Uri.joinPath(
                runFolderPath,
                "makeandstart.bat",
            );
            try {
                ...
                vscode.window.showErrorMessage(
                    `No makeandstart.bat found in run catalog for ${application.name}`,
                );
                return;
            }
            const existingTerminal = vscode.window.terminals.find(
                (t) => t.name === `Septic: ${application.name}`,
            );
            const terminal =
                existingTerminal ||
...
export function registerCommandMakeConfig() {
    vscode.commands.registerCommand(
        "septic.applicationTree.make",
        async (e: ApplicationTreeItem) => {
            vscode.window.terminals
                .find((t) => t.name === `Septic: Make scg config`)
                ?.dispose();
...
export function registerCommandGetFunctions(
    context: vscode.ExtensionContext,
    client: LanguageClient,
) {
    vscode.commands.registerCommand("septic.getFunctions", async () => {
        ...
        if (!editor || !editor.document.fileName.endsWith(".cnfg")) {
            vscode.window.showInformationMessage(
                "No .cnfg file is currently open.",
            );
            return;
        }
        const uri = editor.document.uri.toString();
        const functions = await client.sendRequest(protocol.getFunctions, {
            uri,
        });
