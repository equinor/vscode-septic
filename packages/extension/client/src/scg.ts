...
import { scgConfigFromYAML, ScgConfigSchema } from "@equinor/septic-config-lib";
...
export class ScgConfig {
    ...
    async save(): Promise<void> {
        await vscode.workspace.fs.writeFile(
            ...
            Buffer.from(yaml.dump(this.config)),
        );
    }
    ...
    public addTemplate(
        ...
        elementToInsertAfter: string | undefined = undefined,
    ): void {
        if (elementToInsertAfter) {
            const index = this.config.layout.findIndex(
                (item) => item.name === elementToInsertAfter,
            );
            if (index !== -1) {
...
export async function readScgConfig(
    filePath: string,
): Promise<ScgConfigSchema | undefined> {
    const uri = vscode.Uri.file(filePath);
    const document = await vscode.workspace.openTextDocument(uri);
    try {
        return scgConfigFromYAML(document.getText());
    } catch (error) {
        console.log(`Error parsing SCG config from file ${filePath}: ${error}`);
        return undefined;
    }
}
...
export async function isScgConfig(filePath: string): Promise<boolean> {
    try {
        const uri = vscode.Uri.file(filePath);
        const document = await vscode.workspace.openTextDocument(uri);
        scgConfigFromYAML(document.getText());
        return true;
    } catch {
        return false;
    }
...
export class ScgSource {
    ...
    private async load(): Promise<string[][] | undefined> {
        ...
        try {
            ...
            const data = linesTrimmed.map((line) =>
                line.split(this.delimiter).map((value) => value.trim()),
            );
            return data;
        } catch {
    ...
    async write(): Promise<void> {
        ...
        try {
            await vscode.workspace.fs.writeFile(
                uri,
                Buffer.from(
                    data.map((row) => row.join(this.delimiter)).join("\n"),
                ),
            );
        } catch {
    ...
    addRow(index: string, values: string[]): void {
        ...
        if (values.length !== header.length - 1) {
            throw new Error(
                `The number of values does not match the number of columns. Expected ${header.length - 1}, got ${values.length}`,
            );
        }
    ...
    addColumn(column: string, values: string[]): void {
        ...
        if (values.length !== this.data.length - 1) {
            throw new Error(
                `The number of values does not match the number of rows. Expected ${this.data.length - 1}, got ${values.length}`,
            );
        }
        header.push(column);
