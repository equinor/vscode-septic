...
export class SepticCnfgFormatter {
    ...
    private formatObject(object: SepticObject) {
        ...
        if (editedFlag) {
            ...
            const currentLineText = this.doc.getText({
                start: createPosition(pos.line, 0),
                end: pos,
            });
            if (lineCommentRegex.test(currentLineText)) {
                const prevLineText = this.doc.getText({
                    start: createPosition(pos.line - 1, 0),
                    end: createPosition(pos.line - 1, 999),
                });
    ...
    private addEdit() {
        ...
        if (!previousElement || this.isAtEnd()) {
            endPos = createPosition(this.doc.lineCount, 99);
        } else {
        ...
        this.addLine();
        const edit = createReplaceEdit(
            this.doc.positionAt(this.start),
            endPos,
            this.lines.join("\n"),
        );
        this.lines = [];
