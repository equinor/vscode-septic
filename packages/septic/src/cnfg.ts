...
export class SepticCnfg implements SepticContext, TextDocument {
    ...
    public parse(cts: CancellationToken | undefined): void {
    ...
    public findObjectFromLocation(
        ...
        if (firstObject && offset < firstObject.start) {
            return undefined;
        }
        let index = 1;
        while (index < this.objects.length) {
            const currentObject = this.objects[index];
            if (currentObject && currentObject.start >= offset) {
...
function calcPvrReferences(obj: SepticObject): SepticReference[] {
    ...
    visitor.variables.forEach((xvr) => {
        ...
        if (parsedAlg.positionsMap.length) {
            ...
            // @ts-expect-error: originalPositions will always have one element
