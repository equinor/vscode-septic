...
export function getFunctionsFromCalcPvrs(
    calcPvrs: SepticObject[],
): SepticFunction[] {
    const nodes = calcPvrs
        .filter((child) => child.identifier?.id)
        .map((child) => {
            return {
                obj: child,
    ...
    const functions: SepticFunction[] = functionNodes.map((node) =>
        createFunctionFromNode(node.name!, node.node, node.depth),
    );
    return functions;
}

function visitNode(
    node: SepticFunctionNode,
    allNodes: SepticFunctionNode[],
    ancestors: string[],
) {
    if (node.visited) {
        return;
...
function createFunctionFromNode(
    name: string,
    node: SepticFunctionNode,
